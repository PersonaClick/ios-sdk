...
/**
 ...
 Mirror of the Android `PersonaClick` facade, adapted to Swift: resolution failures surface as a thrown
 `PersonaClickError` (not exceptions), and `awaitInstance` is a non-throwing callback API for UI.

 ```swift
 // Single shop:
 PersonaClick.initialize(PersonaClickConfig(shopId: "SHOP_ID"))
 try PersonaClick.instance().track(event: .categoryView(id: "1"))

 // Several shops, initialized lazily on first use:
 PersonaClick.register(shops: [PersonaClickConfig(shopId: "shop-a"), PersonaClickConfig(shopId: "shop-b")])
 try PersonaClick.instance(for: "shop-a").track(event: .categoryView(id: "1"))
 ```
 */
public enum PersonaClick {

    /// Shops registered lazily and not yet initialized. Materialized on the first `instance(for:)`.
    private static var pending: [String: PersonaClickConfig] = [:]
    ...
    internal static var pendingPushTracker: (String, PersonaClickConfig, PushEvent, String, String) -> Void
    ...
    /**
     ...
     stay consistent.
     */
    public static func register(shops: [PersonaClickConfig], eagerInit: Bool = false) {
        for config in shops {
            if eagerInit {
                _ = initialize(config)
            } else {
    ...
    /**
     ...
     - Throws: `PersonaClickError.ambiguousShop` when `shopId` is `nil` and more than one shop is registered;
       `PersonaClickError.unknownShopId` when the shop is unknown (nothing registered, or no such shop).
     */
    public static func instance(for shopId: String? = nil) throws -> PersonalizationSDK {
        ...
        case .existing(let resolved):
            guard let sdk = SdkRegistry.shared.byShopId(resolved) else {
                throw PersonaClickError.unknownShopId(resolved)
            }
            return sdk
        case .pending(let resolved):
            return try materialize(shopId: resolved)
        case .notRegistered:
            throw PersonaClickError.unknownShopId(shopId ?? "")
        case .ambiguous:
            throw PersonaClickError.ambiguousShop(registeredShopIds())
        }
    }
    ...
    private static func pendingConfig(for shopId: String) -> PersonaClickConfig? {
        lock.lock()
        defer { lock.unlock() }
