...
/**
 Errors raised by `PersonaClick.instance(for:)` when a shop cannot be resolved.
 ...
 */
public enum PersonaClickError: Error {
...
extension PersonaClickError: LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .unknownShopId(let shopId):
            ...
            return "\(subject) Call PersonaClick.initialize(...) or PersonaClick.register(shops:) first."
        case .ambiguousShop(let shopIds):
            return "More than one shop is registered — call PersonaClick.instance(for:) with an explicit " +
