...
public class NotificationService: NotificationServiceProtocol {
  ...
  /// Retained for source compatibility. Notification tracking now routes through `PersonaClick.handlePush`
  ...
  public func didReceiveRemoteNotifications(
    ...
    // Track against the shop the push names (multi-instance): `PersonaClick.handlePush` resolves the target
    ...
    PersonaClick.handlePush(userInfo, event: .delivered)
  ...
  private func pushProcessing(userInfo: [AnyHashable: Any]) {
    ...
    PersonaClick.handlePush(userInfo, event: .clicked)
  ...
  private func pushRetrieved(userInfo: [AnyHashable: Any]) {
    ...
    PersonaClick.handlePush(userInfo, event: .received)
  ...
  public func extractTypeAndCode(from userInfo: [AnyHashable: Any]) -> (type: String, code: String)? {
    // Shared with `PersonaClick.handlePush` so both paths accept the same payload shapes.
